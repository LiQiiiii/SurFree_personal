...
def distance(a, b):
    return (a - b).flatten(1).norm(p=np.inf,dim=1)
...
class SurFree_inf():
    ...
    def __call__(self, model, X, labels, starting_points=None, **kwargs):
        ...
        self._nqueries = torch.zeros(len(X)).to(X.device) # 每个样本的查询次数
        ...
        # Check if X are already adversarials. images_finishied = TRUE if already adversarials.
        self._images_finished = model(X).argmax(1) != labels

        print("Already advs: ", self._images_finished.cpu().tolist())

        print("images_finished: ", self._images_finished)
        print("X.shape: ", X.shape)
                                        #(tensor([False, False], device='cuda:0'),torch.Size([2, 3, 32, 32]))
        self.best_advs = torch.where(atleast_kdim(self._images_finished, len(X.shape)), X, self.best_advs) # 找到最初的对抗样本点
        print("self.best_advs before binary_search: ", self.best_advs.shape)

        self.best_advs = self._binary_search(self.best_advs, boost=True)
        print("self.best_advs after binary_search: ", self.best_advs.shape)
        print()

        # Initialize the direction orthogonalized with the first direction
        fd = self.best_advs - self.X # u向量的方向
        ################################################################################
        # for i, v in enumerate(fd):
        #     if v.norm(p=np.inf)<0.031:
        #     num_queery[i]=
        ################################################################################
        self._directions_ortho = {i: v.unsqueeze(0) / v.norm() for i, v in enumerate(fd)} # u向量对应的坐标基
        print("self.directions_ortho: ", self._directions_ortho)

        # Load Basis, 用dct找到其他的基
        self._basis = Basis(self.X, **kwargs["basis_params"]) if "basis_params" in kwargs else Basis(self.X)
        # print("self.basis: ", self._basis)
        # print("self.basis: ", self._basis.shape)
        # print("type of X: ", type(self.X))

        #LiQi: print some info:
        print("X: ", self.X.shape)
        # print("y: ", self.labels)

        for step_i in range(self._steps): # 迭代steps次
            print("Step:", step_i)
            # Get candidates. Shape: (n_candidates, batch_size, image_size)
            candidates = self._get_candidates() # 得到一系列的潜在z*(theta)
            print("candidates: ",candidates.shape)
            candidates = candidates.transpose(1, 0)
            print("candidates transpose: ", candidates.shape)     

            best_candidates = torch.zeros_like(self.best_advs)
            # print("best_candidates: ", best_candidates)
            print("candidates[i]: ", candidates[0]) # 1*3*32*32

            for i, o in enumerate(self.X):
                o_repeated = torch.cat([o.unsqueeze(0)] * len(candidates[i]), dim=0) # 每一个o(也就是原始样本)复制n次
                print("o: ", o)
                print("o.unsqueeze(0): ", o.unsqueeze(0))
                print("len(candidates[i]): )", len(candidates[i]))
                print("[o.unsqueeze(0)] * len(candidates[i]): ", [o.unsqueeze(0)] * len(candidates[i]))
                print("o_repeated: ", o_repeated) # repeated的意思是不断尝试

                index = distance(o_repeated, candidates[i]).argmax() # 得到与原始样本距离最大的对抗样本
                print("distance(o_repeated, candidates[i]): ", distance(o_repeated, candidates[i]))
                print("index: ", index)

                best_candidates[i] = candidates[i][index] # 得到当前图片的最优对抗样本
                print("best_dandidates[i]: ", best_candidates[i])
            is_success = distance(best_candidates, self.X) < distance(self.best_advs, self.X) 
            self.best_advs = torch.where( # 因为每次都是在一个超平面内确定是否更优，所以每次找到更优的只是在某一些维度进行更新
    ...
    def _get_candidates(self):
        ...
        distances = (self.X - self.best_advs).flatten(1).norm(p=np.inf,dim=1)
        while (epsilon == 0).any():
        ...
        if self.with_interpolation:
            d = (self.best_advs - self.X).flatten(1).norm(p=np.inf,dim=1)
            delta = (self._binary_search(candidates[1],  boost=True) - self.X).flatten(1).norm(p=np.inf,dim=1)
        ...
        print("epsilon in candidates: ", epsilon)
        return candidates

    def _get_evolution_function(self, direction_2):
        distances = (self.best_advs - self.X).flatten(1).norm(p=np.inf,dim=1, keepdim=True)
...
class Basis:
    ...
    def get_vector(self, ortho_with = None, indexes= None) -> torch.Tensor:
        ...
        r: torch.Tensor = self._function_generation(indexes) # indexes 传进来的是还没有找到正交基的样本的索引， r应该是通过dct随机生成的方向
        # print("r in get_vector: ", r)
        vectors = [
            self._gram_schmidt(r[i], ortho_with[i]) if ortho_with[i] is not None else r[i]
            for i in range(len(self.X))
        ] # 得到一组正交基
        # print("vectors in get_vector: ", vectors)
        vectors = torch.cat([v.unsqueeze(0) for v in vectors], dim=0)
        norms = vectors.flatten(1).norm(dim=1)
        vectors /= atleast_kdim(norms, len(vectors.shape)) # 得到一组单位正交基
        return vectors

    def _gram_schmidt(self, v: torch.Tensor, ortho_with: torch.Tensor):
        v_repeated = torch.cat([v.unsqueeze(0)] * len(ortho_with), axis=0) # 将v复制到和图像数量一样
